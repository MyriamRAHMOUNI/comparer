...
def plot_delta_Neq(dfd,nom_sortie):
    pyplot.plot('residus', 'Delta_Neq_abs', data = dfd, color = 'green', linewidth = 2)
...
class Data_count:
    ...
    def calcule_et_plot_delta_PB(self, data_count_2, nom_sortie):
        ...
        pyplot.plot(delta_pb, linewidth = 2)
        pyplot.xlabel('Residus')
...
if __name__ == "__main__":
    if sys.argv[1] == "help":
       help()
        ...
        print("\nInputed files:", sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4],'\n\n' "Output file:", sys.argv[5]+"_Delta_Neq.png", sys.argv[5]+"_Delta_Neq.txt", sys.argv[5]+"_Delta_PB.png", sys.argv[5]+"_Delta_PB.txt", sys.argv[5]+"_Map_delta_PB.txt")
	    ...
	    # Création des objets Fichier 
        Neq_1 = Fichier(df_Neq_1.shape[0], df_Neq_1.shape[1], str(df_Neq_1.columns.values))
        count_1 = Fichier(df_count_1.shape[0], df_count_1.shape[1], str(df_count_1.columns.values))
        Neq_2 = Fichier(df_Neq_2.shape[0], df_Neq_2.shape[1], str(df_Neq_2.columns.values))
        count_2 = Fichier(df_count_2.shape[0], df_count_2.shape[1], str(df_count_2.columns.values))    

        print("\n-------Vérification format")
        
        # Verification format des objets Fichier : longueur des séquences PB et entêtes 
	    ...
	    #Création de la dataframe delta_Neq
        df_delta_Neq = delta_Neq(df_Neq_1, df_Neq_2, str(sys.argv[5]))
          
        print("\n-------Fichier "+ str(sys.argv[5]) + "_Delta_Neq.txt crée")
	
	    #Courbe delta_Neq en fonction des résidus         
        plot_delta_Neq(df_delta_Neq, str(sys.argv[5]))
        print("\n-------Figure "+ str(sys.argv[5]) + "_Delta_Neq.png crée")
        
        # Création des objets Data_count 
        ...
        # Calclule des delta PB et création des courbes et de la map         
        data_count_1.calcule_et_plot_delta_PB(data_count_2, str(sys.argv[5]))
       
        print("\n-------Fichier "+ str(sys.argv[5]) +"_matrice_Delta_PB.txt crée")
        print("\n-------Fichier "+ str(sys.argv[5]) + "_Delta_PB.png crée")
        print("\n-------Figure " + str(sys.argv[5]) + "_MAP_delta_PB.png crée")
        print("\n-------Figure "+ str(sys.argv[5]) + "_Delta_Neq.txt crée")
